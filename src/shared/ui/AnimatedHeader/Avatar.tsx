...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const [{ loading, failed }, dispatch] = useReducer(
    (
      state: AvatarReducerState,
      action: AvatarReducerActions
    ): AvatarReducerState => {
  ...
  if (failed)
    return <Ionicons name="person-circle-outline" size={wh} color="black" />;
  ...
  return (
    <>
      {loading && (
        <View
          style={[
            { height: wh, width: wh, borderRadius: wh / 2 },
            styles.loader,
          ]}
        >
